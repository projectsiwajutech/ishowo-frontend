...
exports.SaleNewComponent = void 0;
...
var SaleNewComponent = /** @class */ (function () {
    function SaleNewComponent(saleService, categoryService, productService, libraryService, locStorService, ngxService, router, modalService, constantService) {
    ...
    //ngOnInit
    SaleNewComponent.prototype.ngOnInit = function () {
    ...
    SaleNewComponent.prototype.ngAfterViewInit = function () {
        this.onLoadFocus();
    };
    //ngOnChanges
    SaleNewComponent.prototype.ngOnChanges = function () {
        this.user = this.locStorService.getUser();
        this.param = new ProductParam_1.ProductParam();
        this.getStockView();
        this.item = new sale_1.Sale();
    ...
    //test de limitation
    SaleNewComponent.prototype.inputValidator = function (event) {
        var pattern = /[0-9]/;
    ...
    //add product line
    SaleNewComponent.prototype.addProductLine = function (obj) {
        //check if undefined
    ...
    //remove product line
    SaleNewComponent.prototype.removeProductLine = function (index) {
        this.item.lines.splice(index, 1);
        this.lineSize = this.item.lines.length;
    }; //fin removeProductLine
    //can save devis
    SaleNewComponent.prototype.canSaveDevis = function () {
        var stateProductLines = (this.item.lines === null || this.item.lines === undefined || this.item.lines.length === 0);
        var isSavingSale = this.isSaving;
        if (stateProductLines === true || isSavingSale === true) {
            return false;
        }
        else {
            return true;
    ...
    //get Net à Payer
    SaleNewComponent.prototype.getNap = function (form) {
    ...
    // get Reliquat
    SaleNewComponent.prototype.getReliquat = function () {
        var total = this.NAP;
    ...
    //get Total TVA
    SaleNewComponent.prototype.getTVA = function () {
        if (this.item === undefined || this.item === null) {
            return 0;
        }
        if (this.item.lines === undefined || this.item.lines === null) {
    ...
    //fin get Total TVA
    // get RestToPaid
    SaleNewComponent.prototype.getRestToPaid = function () {
        var total = this.NAP;
        var calc = this.numPadAmount - total;
        if (calc < 0) {
    ...
    //get total
    SaleNewComponent.prototype.getTotal = function () {
        if (this.item === undefined || this.item === null) {
            return 0;
        }
        if (this.item.lines === undefined || this.item.lines === null) {
    ...
    //create object
    SaleNewComponent.prototype.saveSale = function () {
        var _this = this;
        ...
        else if (this.numPadAmount === undefined || this.numPadAmount === null) {
            this.libraryService.onWarning('veuillez renseignez le Montant reçu!', 1200, 'top-end');
            return;
        }
        else if (this.getRestToPaid() !== 0) {
            this.libraryService.onWarning('Le montant saisi est incorrect!', 1300, 'top-end');
            return;
        }
        ...
        if (this.locStorService.readFromSession('Customer') === null) {
            ...
            this.customer.solde = this.getRestToPaid();
        }
        //statut indicators
        this.isSaving = true;
        this.isCompleted = false;
    ...
    //create object
    SaleNewComponent.prototype.saveDevis = function () {
        var _this = this;
        //conditions reunies ou non
        if ((this.item.lines === null || this.item.lines === undefined || this.item.lines.length === 0)) {
            this.libraryService.onWarning('Veuillez ajouter un Produit!', 1100, 'top-end');
    ...
    SaleNewComponent.prototype.getStockView = function () {
        var _this = this;
        this.stock_view = [];
    ...
    SaleNewComponent.prototype.getCategories = function () {
        var _this = this;
        this.categories = [];
    ...
    //liste les ventes
    SaleNewComponent.prototype.listSale = function () {
        var link = ['/app/sales_list'];
        this.router.navigate(link);
    }; //fin listSale
    //add breakOrNot
    SaleNewComponent.prototype.addBreakOrNot = function (itemName) {
    ...
    //change Product Tax
    SaleNewComponent.prototype.changeTax = function (id, tax) {
    ...
    SaleNewComponent.prototype.updateAllProducts = function () {
        var _this = this;
        this.item.lines.forEach(function (element) {
            _this.updateTax(element.id, element.tax);
        });
    ...
    //update Product Tax
    SaleNewComponent.prototype.updateTax = function (id, tax) {
        var searchProdList = this.item.lines.filter(function (x) { return x.id === id; });
        var searchProdIndex = this.item.lines.indexOf(searchProdList[0]);
        var Original_Price = this.item.lines[searchProdIndex].p_vente * this.item.lines[searchProdIndex].quantite;
        this.item.amount_remise = this.REMISE;
    ...
    SaleNewComponent.prototype.IncrementQuantity = function (obj) {
        //check if undefined
        if (this.item.lines === undefined || this.item.lines === null) {
            this.item.lines = [];
        }
    ...
    SaleNewComponent.prototype.DecrementQuantity = function (obj, quantity) {
        //check if already added
        if (quantity == 1) {
    ...
    SaleNewComponent.prototype.CheckQuantity = function (obj, quantity) {
        //check if already added
        var searchProdList = this.item.lines.filter(function (x) { return x.id == obj.id; })[0];
        var searchProdIndex = this.item.lines.indexOf(searchProdList);
        //check qty
    ...
    //add new customer
    SaleNewComponent.prototype.AddNewCustomer = function () {
    ...
    // Reset NumPad
    SaleNewComponent.prototype.ResetNumPad = function () {
        this.numPadAmount = null;
    }; //fin Reset NumPad
    //add NumPadAdd
    SaleNewComponent.prototype.NumPadAdd = function (num) {
    ...
    //add NumPadAdd
    SaleNewComponent.prototype.RemoveOnPad = function () {
        var inputVal = document.getElementById("numPadAmount").value;
    ...
    SaleNewComponent.prototype.checkIfCustomerExist = function () {
    ...
    //delete Customer
    SaleNewComponent.prototype.deleteCustomer = function () {
        localStorage.removeItem('Customer');
    };
    //fin delete Customer
    //give up Sale
    SaleNewComponent.prototype.giveUpSale = function () {
        localStorage.removeItem('Customer');
        this.item = new sale_1.Sale();
        this.lineSize = 0;
        this.ResetNumPad();
    ...
    SaleNewComponent.prototype.getSelectedCustomer = function () {
        var selectedCustomer = this.locStorService.readFromSession('Customer');
        if (selectedCustomer === null) {
            ...
            if (this.customer.social_reason === "" || this.customer.social_reason == null) {
                var customerInfo = this.customer.nom + ' ' + this.customer.prenom;
                return customerInfo;
            }
            else {
                var customerInfo = this.customer.social_reason;
                return customerInfo;
            }
        }
    }; //fin get selected Customer
    //get paramsMecef
    SaleNewComponent.prototype.getParamsMecef = function () {
        var _this = this;
    ...
    //reset filter form
    SaleNewComponent.prototype.resetFilterForm = function () {
        this.param = new ProductParam_1.ProductParam();
    };
    //fin reset form
    //printData of Last sale
    SaleNewComponent.prototype.printData = function () {
    ...
    SaleNewComponent.prototype.OnchangeStatut = function ($event) {
        this.ResetNumPad();
        this.is_checked = $event.target.checked;
    }; //fin OnchangeStatut of Devis Switch
    //confirmProdRemoveModal
    SaleNewComponent.prototype.confirmProdRemoveModal = function (item, itemIndex) {
    ...
    //focus barrecode Onload
    SaleNewComponent.prototype.onLoadFocus = function () {
        document.getElementById("code_barre").focus();
    };
    __decorate([
        core_1.Input()
    ], SaleNewComponent.prototype, "subject");
    __decorate([
        core_1.Output()
    ], SaleNewComponent.prototype, "created");
    __decorate([
        core_1.ViewChildren('productsList')
    ], SaleNewComponent.prototype, "listItems");
    SaleNewComponent = __decorate([
        core_1.Component({
    ...
    return SaleNewComponent;
}());
exports.SaleNewComponent = SaleNewComponent;
