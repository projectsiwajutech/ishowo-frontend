...
export class SaleNewComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  visible: boolean = false; setGroup: string; setAIB: string;
  ...
  saveSale() {
    ...
    if ((this.item.lines === null || this.item.lines === undefined || this.item.lines.length === 0)) {
      ...
      this.libraryService.onWarning('veuillez renseignez le Montant reçu!', 1200, 'top-end'); return;
    } else if (this.getRestToPaid() !== 0) {
      this.libraryService.onWarning('Le montant saisi est incorrect!', 1300, 'top-end'); return;
    }
  ...
  getSelectedCustomer() {
    ...
    if (selectedCustomer === null) {
      ...
      if (this.customer.social_reason === "" || this.customer.social_reason == null) {
        ...
        let customerInfo = this.customer.social_reason;
        return customerInfo;
      }
    }
